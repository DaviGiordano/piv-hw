...
class PoseGraph:
    def __init__(self, image_nodes, nodes_match):
        ...
        #: Build a graph of edges (src_idx, dst_idx, transform, error)
        self.edges = self._build_pose_graph()

        # Initialize transforms (Identity)
    ...
    def _build_pose_graph(self):
        ...
        for (i, j), match in self.nodes_match.items():
            if i != j:
                transform_icp = match.transform_icp
    ...
    def incremental_registration(self, base_node_idx=0):
        """
        Sort edges by registration error. Start with
        'base_node_idx' as the root (pose = identity).
